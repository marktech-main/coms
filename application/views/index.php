...
if($this->privilege['is_supervisor']) { ?>
<style type="text/css">
  tr.DONE {
      display: table-row;
  }
                        ...
                        <a class="queue-table-filter" data-filter="SUCCESSFUL">
                          <div class="panel-heading">
                              <div class="row">
                                  ...
                                  <div class="col-xs-9 text-right">
                                      <div class="huge" name="div_cs_total_successful"></div>
                                      <div>Successful</div>
                                  </div>
                              </div>
                          </div>
                        ...
                        <a class="queue-table-filter" data-filter="CANCELLED">
                          <div class="panel-heading">
                              <div class="row">
                                  ...
                                  <div class="col-xs-9 text-right">
                                      <div class="huge" name="div_cs_total_cancelled"></div>
                                      <div>Cancelled</div>
                                  </div>
                              </div>
                          </div>
                        ...
                        <a class="queue-table-filter" data-filter="CANCELLED">
                          <div class="panel-heading">
                              <div class="row">
                                  ...
                                  <div class="col-xs-9 text-right">
                                      <div class="huge" name="div_total_cancelled"></div>
                                      <div>Cancelled</div>
                                  </div>
                              </div>
                          </div>
...
$(document).ready(function() {
  ...
  ?>
    ...
    var dt_toolbar = "<'dt-toolbar'<'col-xs-12 col-sm-6'><'col-sm-6 col-xs-12 hidden-xs'l>r>"+
        "t"+
        "<'dt-toolbar-footer'<'col-sm-6 col-xs-12 hidden-xs'i><'col-xs-12 col-sm-6'p>>";
  <?php
        }else{
  ...
  window.oTable = $('#dt_request_transaction').dataTable({
      ...
      "aoColumns": [
          ...
          { "mData": "created_by_name", "sClass": "affiliate list"},
          { "mData": "request_time" },
          ...
          { "mData": "come_from", "sClass": "dt-center" },
          {
              ...
              "render":function(data, type, full, meta){
                  $html='';
                  $html +='<a class="btn btn-success btn-xs verifyBtn"  action="edit" data-target="'+full.transaction_id+'" data-status="'+full.status+'"><i class="fa fa-cog fa-fw fa-pulse"></i>Verify</a>';
                  return $html;
              },
              "sClass": "center",
              targets: 'no-sort',
  ...
  <?php
        if(!is_cs_team(decrypt($this->session->userdata('user_data'))['user_role'])){
  ?>
  $('#dt_request_transaction_filter').html('<label>Division: <select name="division_filter"><option value=""> ALL </option></label>');
...
$(document).on('click', '[action="edit"]', function(){
	var redirect = 'update';
	// check if the current status is queue then disable verify button
	if($(this).data('status') == 'QUEUE') {
	  socket.emit('disable_verify_button', {transaction_id: $(this).data('target')});
	}
	$.redirectPost(redirect, {transaction_id: $(this).data('target'), state: '1'});
});

socket.on('disabled_verify_button', function(data){
   $('a[data-target="'+data.transaction_id+'"]').attr("disabled", true);
});
